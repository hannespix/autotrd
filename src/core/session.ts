...
import { bucketEnd, dayKeyFor, sessionBounds, type Calendar, MIN } from './time.ts';
...
export function buildSessionInfo(
  ...
  // Bars seit Open (inkl. aktueller): rückwärts zählen, solange derselbe Handelstag.
  let barsSinceOpen = 1;
  for (let k = i - 1; k >= 0; k--) {
    if (dayKeyFor(bars.t[k]!, assetClass) !== day) break;
    barsSinceOpen++;
  }
