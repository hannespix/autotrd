...
| `backtest/` | Portfolio-Simulator (Fills am nächsten Open, Stop vor Ziel; Fahrplan `ParameterWechsel` je Symbol — ab `ab` neue Parameter samt Indikatoren, `null` ⇒ kein Einstieg, offene Position `unmanaged` geschlossen; ohne Fahrplan bitgleich), Kosten, Metriken (Sharpe/Sortino/PSR/DSR), Marktbezug (`marktbezug.ts`: kaufen und halten als Maßstab unter jedem Holdout — kein Gate, aber ohne ihn liest man Markt als Kante). |
...
| `optimize/` | Walk-Forward — **die OOS-Kette ist EINE Simulation** (`optimizer.oosChain: continuous`, seit 18.09.2026, `docs/wissen/vorregistrierung/2026-09-18-durchgehende-oos-kette.md`): `durchgehendeKette` läuft von `oosStart` des ersten bis `oosEnd` des letzten Folds, Parameter- und Korbwechsel an den Fold-Grenzen per Fahrplan, Scheiben auf E₀ normiert (`kettenScheiben` — Equity UND Geldbeträge der Trades, Nachtrag `2026-09-18-nachtrag-massstab-auf-der-kette.md`), Stress ebenso, der SPY-Maßstab von `beats_market` auf derselben Tagesachse (`marktKetteAufAchse`, Wächter im Gate); `per_fold` (leeres Buch je Fold, Buchgewinne an Fold-Enden — Prüfbefund K4; laut Prüfer M1 zugleich ein Neustart-Preis je Grenze) nur für Vergleichsläufe. Robustheits-Gates (darunter `beats_market`: schlägt die OOS-Kette den Sharpe von kaufen-und-halten? Gegen die Benchmark, nicht den Korb; ohne Benchmark gilt die Kasse — das Gate wird nie vakant), Champion/Challenger, Report. **Festkandidaten** (`optimizer.fixedCandidates`): vorregistrierte Parametersätze ohne Suche über alle Folds durch dieselben Gates, DSR laut „nicht anwendbar", keine Sonderbehandlung; je Kandidat eine Maßstab-Zeile (OOS-Sharpe, MaxDD, Trades je Monat gegen SPY über dieselben OOS-Fenster — und gegen den liegengelassenen Korb je Fold, Prüfbefund M6), P(SR > Latte) in der `beats_market`-Notiz, Symbol-Konzentration (M9) und Mitglieder je Korbstand (M8) im Bericht. |
