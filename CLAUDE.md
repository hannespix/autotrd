...
| `strategy/` | Indikatoren (kausal) und Vorlagen. Symbolweise: `trend_donchian`, `momentum_pullback`, `mean_reversion`, `orb_breakout`. Querschnittlich (fragt den KORB, nicht das Symbol): `cross_sectional_momentum` — die Rangliste baut `decide()`, nie die Strategie selbst. Allokation (`regime_allocation`): Regime + relative und absolute Stärke, monatlich, Gewicht = Zielvolatilität / realisierte Volatilität über `Decision.weight` (Sizing in `risk/sizing.ts`, ein Pfad), weiter Katastrophen-Stop beim Broker, kein Trailing. |
