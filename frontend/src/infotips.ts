...
export const INFO: Record<string, { t: string; d: string }> = {
  ...
  minEdgeMultiple: {
    t: 'Kostenschwelle',
    d: 'Der wichtigste Filter des Systems — und der, den es am längsten nicht gab. Jeder Trade kostet Gebühren plus Spread, hin und zurück: 0,1 % bei US-Aktien, bis 0,5 % bei Krypto. Bis zum 28.07. spielte das bei der ENTSCHEIDUNG keine Rolle; gehandelt wurde nach Signal, gezahlt wurde hinterher. Das Ergebnis über 297 echte Trades: 1 774 $ Gebühren gegen 656 $ Brutto-Verlust — die Reibung war das 2,7-Fache des eigentlichen Ergebnisses. Jetzt prüft die Engine vorher: Bewegt sich dieses Instrument in der Mindest-Haltedauer überhaupt weit genug, um die Kosten zu schlagen? Gerechnet mit der ATR und der Wurzel aus der Haltedauer (über vier Kerzen verdoppelt sich die erwartete Bewegung, sie vervierfacht sich nicht). Steht hier 3, muss die erwartete Bewegung dreimal so groß sein wie die Kosten — der Trade muss also auch dann tragen, wenn zwei von drei Versuchen danebengehen. Höher = weniger, aber lohnendere Trades. 0 schaltet den Filter ab (nicht empfohlen).',
  },
  newsVeto: {
    t: 'News-Veto',
