...
export const INFO_DE: Record<string, Tip> = {
  ...
  symbolProfil: {
    t: 'Symbolprofil',
    d: 'Das Profil erklärt, was die Engine für dieses Symbol tut und warum; handeln tut nur decide() mit Champion oder Basis. Es entsteht jede Nacht am Ende des Optimierer-Laufs aus geschlossenen Tagesbars — kausal, ohne Blick nach vorn — und trägt je Symbol: die Anlageklasse (feste Zuordnung: Basis-ETFs nach GTAA-Klasse, Benchmark, sonst Aktie mit Sektor), den Trendzustand (Schluss über oder unter dem gleitenden Mittel, und seit wie vielen Handelstagen), den Rang im Korb (dieselbe Rangbildung, die auch die Engine benutzt — nur für Querschnitts-Taktiken; der Korb ist der der PLATTFORM, jede Rangzahl nennt seine Mitglieder — deine Engine rangiert nur deine Symbolauswahl, die Basis kommt für alle als Block), die Volatilität (63 Handelstage, annualisiert), das Momentum, die Stop-Distanz der zugewiesenen Taktik (fester Katastrophen-Stop bei der Basis, ATR-Vielfaches beim Alpha-Champion), die Liquidität (Median-Dollarumsatz je Tag, dieselbe Kennzahl wie die nächtliche Universumswahl), die zugewiesene Taktik mit Quelle und Grund (Alpha-Champion vor Basis vor „keine" — wörtlich die Wahl der Engine, keine zweite Ableitung) und die letzte Bewertung (Lauf-Nummer, Messzeitpunkt, Config-Commit). Die erwartete Haltedauer ist eine GEMESSENE Größe der Taktik; solange der Champion sie nicht trägt, steht dort „unbekannt" — geschätzt wird nichts. Das Profil gilt plattformweit; deine Symbolauswahl, dein Basis-Schalter, dein Positionsdeckel und Sperren oder Not-Aus deines Kontos stehen nicht darin. Gehört es zu einem älteren Champion als dem aktuellen (Profil-Schritt der Nacht gescheitert), sagt der Kopf der Karte das.',
  },
  // ── Konto ──
  resetWallet: {
...
export const INFO_EN: Record<string, Partial<Tip>> = {
  ...
  symbolProfil: {
    t: 'Symbol profile',
    d: 'The profile explains what the engine does for this symbol and why; only decide() with champion or base actually trades. It is written every night at the end of the optimiser run from closed daily bars — causally, with no look ahead — and carries per symbol: the asset class (fixed mapping: base ETFs by GTAA class, benchmark, otherwise stock with sector), the trend state (close above or below the moving average, and for how many trading days), the rank in the basket (the same ranking the engine uses — only for cross-sectional tactics; the basket is the PLATFORM basket and every rank names its members — your engine ranks only your symbol selection, the base is added for everyone as a block), volatility (63 trading days, annualised), momentum, the stop distance of the assigned tactic (fixed catastrophe stop for the base, ATR multiple for the alpha champion), liquidity (median dollar turnover per day, the same metric as the nightly universe selection), the assigned tactic with source and reason (alpha champion before base before “none” — literally the engine\'s choice, not a second derivation) and the last assessment (run number, measurement time, config commit). The expected holding period is a MEASURED property of the tactic; as long as the champion does not carry it, it reads “unknown” — nothing is estimated. The profile is platform-wide; your symbol selection, base switch, position cap and any locks or kill switch of your account are not part of it. If it belongs to an older champion than the current one (the nightly profile step failed), the card header says so.',
  },
  resetWallet: {
    t: 'Reset account',
