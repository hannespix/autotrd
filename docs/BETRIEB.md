...
| 09:30–16:00 | Je geschlossener Bar im Strategie-Zeitrahmen (`timeframe`, Default 5 min, plus `engine.barGraceSec` Karenz): Snapshot bauen → `decide()` → Orders. Keine Einstiege in den ersten `session.noEntryFirstMin` (5) und letzten `session.noEntryLastMin` (30) Minuten. Intraday-Strategien werden am **letzten Entscheidungspunkt vor der Frist** glattgestellt (`minutesToClose − timeframe < session.flattenBeforeCloseMin`; bei 15-Minuten-Bars also um 15:45, bei 5-Minuten-Bars um 15:55) — der Fill liegt am Open der Folgebar, nie am Schlusskurs. |
| nach 16:00 | Exits und Stop-Nachzüge, die erst nach Schluss entschieden werden (letzte Tagesbar, Halt), werden **zurückgestellt** (`deferredIntents` in `state.json`): Die Schutz-Beine bleiben liegen, die Ausführung folgt beim ersten Tick nach der nächsten Eröffnung + Karenz. Eine Tages-Marktorder in die Nacht zu legen und dafür den Stop zu stornieren, gibt es nicht. |
...
| `run` | Die Engine: Streams verbinden, Buch abgleichen, Bars schließen, entscheiden, Orders routen. Läuft, bis SIGINT/SIGTERM kommt. Startet nicht, wenn kein Symbol handelbar ist (kein Champion, `allowWithoutChampion: false`) — und nicht, wenn Universum ∪ Basis-Korb (inkl. Benchmark) bei `broker.feed: iex` mehr als 30 Symbole sind: Der IEX-Datenstrom lehnt das Abonnement mit 405 ab, und ohne Datenfrische gäbe es still keinen einzigen Einstieg (`universe.maxSymbols` verkleinern oder `sip`). `doctor` warnt vorher. |
