...
| `universe` | Handelsuniversum aus `universe.candidates` wählen: Tagesbars für den ganzen Pool, Rang nach Median-Dollarumsatz über 60 Handelstage, die liquidesten `universe.maxSymbols` gewinnen. Schreibt `universe.json` und einen Bericht nach `reports/`. **Nie nach Ertrag** — eine Auswahl nach vergangener Rendite und anschließende Messung auf denselben Daten ist Selektionsbias in Reinform. Ohne `candidates` tut das Kommando nichts. |
