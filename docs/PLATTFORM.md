...
| Symbolprofil | `meta/symbolProfile` (ein Dokument) | Vom selben Workflow ZULETZT geschrieben (`scripts/publish-profile.mjs`, hinter Champion und Config, hängt am Champion-Schritt): je Symbol Klasse, Trend, Rang im Plattform-Korb, Vol, Momentum, Stop, Liquidität, Taktik der Engine mit Grund, Bewertung. Anzeige, kein Handel; `championUpdatedAt` sagt, zu welchem Champion es gehört (`src/profile/symbolprofile.ts`, `docs/wissen/symbolprofile.md`) |
...
| Öffentlich lesbar | `meta/champion`, `meta/engineConfig`, `meta/health`, `meta/symbolProfile`, `meta/optimizeReports/berichte/*` | Alles andere unter `meta/` (Lease, Depot-Bindungen) ist Serversache (Rules-Allowlist) |
